...
using DataAccessor.Classes.ExceptionClasses;

namespace DataAccessor.Classes
{
    class AccessUtils
    {
        //private static readonly string AppDirectoryPath = AppDomain.CurrentDomain.BaseDirectory;
        //private static readonly string DatabasePath = Path.Combine(AppDirectoryPath, "TestDB.accdb");
        ...
        public static void TablesExist()
        {
            ...
            try
            {
                ...
                if (schemaInformation.Rows.Count != 0)
                {
                    ...
                    {
                        if (name != AccessInfo.tableNames[(int)AccessInfo.TableNames.US_PRPO] & name != AccessInfo.tableNames[(int)AccessInfo.TableNames.MX_PRPO])
                            throw new TableNameMismatchException("The MS Access Tables are named incorrectly.");
                        else
                        {
                            if (name == AccessInfo.tableNames[(int)AccessInfo.TableNames.US_PRPO])
                            {
                            ...
                            if (name == AccessInfo.tableNames[(int)AccessInfo.TableNames.MX_PRPO])
                            {
