...
namespace KPA_KPI_Analyzer
{
    public partial class KPIPurchTemplate : UserControl
    {
        ...
        private void RenderOne()
        {
            ...
            PercNoConf = String.Format("{0:n0}", overallData.kpi.purch.initConfVsPRPlanDate.data.PercentUnconf);
        ...
        public void ViewData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tag != 10)
                {
                    using (DataViewer dv = new DataViewer() { Data = initConfVsPrPlanDateDt })
                    {
                        dv.LoadData();
                        dv.ShowDialog();
                    }
